...
/* create new dynamically allocated node, initialize all values */
list *create_new_list (list_node *list_node_in)
{
    list *new_list = NULL;

    if (!list_node_in) {    /* validate data not NULL */
    ...
    if (!(new_list->head = malloc (sizeof (list_node_in) + 1))) {
        perror ("malloc-new_list->list");
        free (new_list);
        return NULL;
    }
    new_list->head = list_node_in;  /* copy data to new_node->data */
